% Final
\chapter{Seznam funkčních požadavků}\label{chapter:requirements-list}

% Final
V této příloze lze nalézt seznam všech funkčních požadavků aplikace.

% Final
\section{FP-1 Úvodní stránka aplikace}\label{sec:fp-welcome-page}
...
% Final
Tento požadavek vychází z analýzy konkurence, ve které většina aplikací obsahovala kromě stránky pro přihlášení a registraci i úvodní stránku s grafikou či textem, který prezentuje hlavní přínosy aplikace pro uživatele.

% Final
\section{FP-2 Úvodní dotazník a doporučování kurzů}\label{sec:fp-onboarding}
...
% Final
\section{FP-3 Prohlížení a doporučování kurzů}\label{sec:fp-course-browse}
...
% Final
Součástí přehledu kurzů by také měla být sekce s doporučenými kurzy. Tato doporučení by měla být založena na úvodním dotazníku, který uživatel vyplnil při registraci. Uživatel by měl mít možnost dotazník kdykoliv vyplnit znovu a přizpůsobit si tak svoje preference při zapisování kurzů.

% Final
\section{FP-4 Stránka kurzu}\label{sec:fp-course-page}
...
% Final
\section{FP-5 Stránka kapitoly kurzu}\label{sec:fp-course-chapter-page}
...
% Final
\section{FP-6 Certifikáty o dokončení kurzu}\label{sec:fp-course-certificate}
...
% Final
\section{FP-7 Cesty učení}\label{sec:fp-learning-paths}
...
% Final
Uživatel by měl mít možnost si kariérní cestu zapsat. Pokud je již uživatel do kariérní cesty zapsán, měl by mít možnost na stránce cesty jednoduše pokračovat v učení se nadcházející lekce. Dále by měl mít uživatel možnost se z kariérní cesty odhlásit.
...
% Final
\section{FP-8 Správa zapsaných kurzů a cest učení}\label{sec:fp-course-management}
...
% Final
\section{FP-9 Studijní plán}\label{sec:fp-study-plan}
...
% Final
\section{FP-10 Učení se a opakování podle studijního plánu}\label{sec:fp-study-plan-learning}
...
% Final
\section{FP-12 Nová struktura učení a opakování}\label{sec:fp-learning-structure}
...
% Final
\section{FP-13 Bloky cvičení}\label{sec:fp-exercise-blocks}
...
% Final
\section{FP-14 Editor kódu}\label{sec:fp-code-editor}
...
% Final
\section{FP-15 Bloky vysvětlení látky}\label{sec:fp-explanation-blocks}
...
% Final
\section{FP-16 Vysvětlení odpovědí ve cvičeních}\label{sec:fp-answer-explanation}
...
% Final
\section{FP-17 Nahlašování problému při učení}\label{sec:fp-report-learning-issue}
...
% Final
\section{FP-18 Statistiky o učení}\label{sec:fp-learning-stats}
...
% Final
\section{FP-19 Stav energie}\label{sec:fp-energy-status}
...
% Final
\section{FP-20 Registrace a přihlášení pomocí účtu Google}\label{sec:fp-google-auth}
...
% Final
\section{FP-21 Stránka nastavení}\label{sec:fp-settings-page}
...
% Final
\begin{itemize}
    \item nastavení účtu uživatele, tedy uživatelského jména, jména, e-mailu, hesla a avatara
...
% Final
\section{FP-22 Tmavý režim}\label{sec:fp-dark-mode}
...
% Final
\section{FP-23 Push Notifikace}\label{sec:fp-push-notifications}
...
% Final
\section{FP-24 Připomínání učení}\label{sec:fp-learning-reminder}
...
% Final
\section{FP-25 Systém zobrazování reklam}\label{sec:fp-advertisement}
...
% Final
\section{FP-26 Nový systém předplatného}\label{sec:fp-subscription-system}
...
% Final
\section{FP-27 Zkušební verze předplatného}\label{sec:fp-free-trial}
...
% Final
\section{FP-28 Přizpůsobení avatara uživatele}\label{sec:fp-avatar-customization}
...
% Final
\section{FP-29 Rozšíření obchodu a odemykání položek}\label{sec:fp-shop-unlock-items}
...
% Final
Dále by v obchodu měla být zavedena nová funkce odemykání položek. Některé položky by měly být dostupné ke koupi pouze, pokud uživatel dosáhl určitého ocenění v aplikaci. Pokud uživatel ještě nedosáhl daného ocenění, měly by se položky uživateli zobrazovat jako zamčené. U těchto položek by měl mít uživatel možnost si zobrazit informace o tom, co musí splnit, aby danou položku odemkl.

% Final
\section{FP-30 Uživatelský profil}\label{sec:fp-user-profile}
...
% Final
\section{FP-31 Zmrazení streaku}\label{sec:fp-streak-freeze}
...
% Final
\section{FP-32 Rozšířená oznámení v aplikaci}\label{sec:fp-extended-notifications} 
...
\begin{itemize}
    ...
    \item uživatel byl přesunut do vyšší ligy z požadavku \nameref{sec:fp-leaderboard}
    \item uživatel byl přesunut do nižší ligy z požadavku \nameref{sec:fp-leaderboard}
...
% Final
\section{FP-33 Truhla s odměnou}\label{sec:fp-treasure-chest}
...
% Final
Po dokončení učení by se uživateli měla zobrazit stránka s obrázkem truhly, kterou bude moci otevřít. Po otevření truhly by měl uživatel obdržet náhodné množství virtuální měny, jako odměnu za své učení. Virtuální měnu bude moci následně využít k nákupu položek v obchodě s virtuální měnou dle požadavku \nameref{sec:fp-shop-unlock-items}.
...
% Final
\section{FP-34 Žebříček uživatelů}\label{sec:fp-leaderboard}
...
% Final
\section{FP-35 Sdílení pokroku mezi uživateli}\label{sec:fp-progress-sharing}
