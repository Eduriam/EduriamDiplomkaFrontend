...
% Write - přidat stránku s dotazníkem pro doporučení kurzů, který by se zobrazil při setupu účtu a který by si uživatel mohl spustit znovu na stránce prohlížení kurzů

% Review
Součástí prohlížení kurzů by měla být sekce s doporučenými kurzy. Tato doporučení by měla být založena jednak na úvodním dotazníku uživatele a jednak na kurzech, které si uživatel sám zapsal.
...
% Write - zde někde zahrnout to, že bych chtěl udělat, aby na hlavní stránce kurzu byly sekce, které by uživatel mohl rozbalit kliknutím. Tak to mají snad všechny analyzované aplikace a je to dobrý, že vidím sekce, na ty můžu kliknout a pak plnit jednotlivé kapitoly.
...
% Review
\subsubsection{FP-5 Studijní plán}

% (původní název požadavku: Učení se napříč kurzy)
% Write - tento požadavek přepsat.. mělo by to být samostatná funkce/stránka "studijního plánu", kde si uživatel bude moci nastavit, jaké kurzy se chce učit a aplikace by mu pak podle toho nastavení každý den doporučovala lekce podle studijního plánu. Mělo by to fungovat trošku podobně jako v Codecademy, ale aby si uživatel mohl zvolit co jak chce umět
% Write - napsat pak, že to vychází z analýzy konkurence Codecademy, která podobnou funkci nabízí 
...
% Write - zde napsat ke cvičení, kde bude uživatel sám kódit do editoru, že tam chceme tlačítko, které mu zformátuje daný kód. To vychází z analýzy konkurence Codecademy
\begin{itemize}
    \item Cvičení 1
    \item Cvičení 2
    \item Cvičení 3
...
% Write
\subsubsection{FP-7 Chatbot}

% Write - napsat, že by při učení měl mít premium uživatel možnost komunikovat s chatbotem, který mu zodpoví na otázky (výraz premium uživatel používat jako roli při popisu requirements, use casů atd.)
...
% Write - tohle nám přišlo s Kubou duplicitní s achievementama, tak bychom se na to možná vykašlali
...
\subsubsection{FP-8 Certifikáty o dokončení kurzu}

% Write - napsat, že chci aby aplikace generovala certifikáty o dokončení kurzů - ten si bude moct uživatel stáhnout a zároveň se mu bude zobrazovat na profilu

\subsubsection{FP-8 Sdílení pokroku mezi uživateli}

% Write - ostatní aplikace mají tento aspekt velice slabý.. Proto by bylo fajn, kdyby uživatelé mohli více sdílet pokrok a achievementy s ostatními, jednoduše ostatní uživatele vyhledávat atd.

\subsubsection{FP-8 Micro interactions}

% Write - zkusit nějak v aplikaci udělat to, že by uživatelé mohli zapnout aplikaci a hned se začít učit (podobně jak tomu je například u sociálních sítí)

\subsubsection{FP-8 Platební systém}

% Write - zde se zamyslet na
...
% write - napsat, že to je inspirováno například aplikací Scrimba a že cílem je aby aplikace působila interaktivnějším a vizuálně poutavějším dojmem. Nicméně že animace by měly být využity v přiměřeném množství.
