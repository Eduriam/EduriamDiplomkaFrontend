...
% Done
\subsubsection{FP-18 Registrace a přihlášení pomocí účtu Google}\label{sec:fp-google-auth}
...
% Review
\subsubsection{FP-21 Push Notifikace}\label{sec:fp-push-notifications}
...
% Done
\subsubsection{FP-23 Systém zobrazování reklam}\label{sec:fp-advertisement}
...
% Done
\subsubsection{FP-24 Nový systém předplatného}\label{sec:fp-subscription-system}
...
% Write - přidat free trial oznámení do extended-notifications
...
% Review
Spuštění zkušební verze by mělo probíhat tak, že aplikace uživateli vysvětlí, jak zkušební doba funguje, uživatel následně zadá své platební údaje a aplikace mu spustí zkušební verzi premium účtu na určitou dobu. Několik dní před koncem zkušební doby pak aplikace oznámí uživateli, že mu bude končit zkušební doba premium účtu. Toto oznámení bude zobrazeno jednak jako oznámení přímo v aplikaci podle požadavku \nameref{sec:fp-extended-notifications} a jednak jako oznámení pomocí e-mailu. Uživatel se bude moci kdykoliv během zkušební doby od předplatného odhlásit. Pokud tak neučiní do konce zkušební doby, bude mu automaticky spuštěno plnohodnotné předplatné.
...
% Done
\subsubsection{FP-30 Rozšířená oznámení v aplikaci}\label{sec:fp-extended-notifications} 
...
% Done
\subsubsection{FP-34 Sdílení pokroku mezi uživateli}\label{sec:fp-progress-sharing}
