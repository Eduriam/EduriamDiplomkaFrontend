...
% Final
Vysvětlování látky pomocí \texttt{ExplanationStudyBlock} je tvořeno animacemi doplněnými o audio nahrávku vysvětlující danou látku. Stránky s vysvětlením látky tak připomínají krátká videa, díky kterým je uživateli vysvětlena látka předtím, než začne plnit jednotlivá cvičení. Klíčovým rozdílem oproti klasickým videím je způsob, jakým jsou tato videa vykreslována.
...
% Final
Z výše zmíněného důvodu jsem se rozhodl vytvořit komponent \texttt{Video}, díky kterému je možné do vysvětlovacích videí vložit video ve standardním formátu, tedy například ve formátu \texttt{mp4}. Díky tomu lze kombinovat oba výše zmíněné přístupy. Pro definici struktury videa je tak možné použít přístup popsaný v této kapitole a pro složitější vizualizace lze na některých místech využít zmíněný komponent \texttt{Video}.
...
\begin{figure}[p]
    ...
    \begin{minipage}{0.8\textwidth}
        \centering
        \includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{./images/diagrams/explanation-model.png}
...
% Final
Poté co byla v rámci učení uživateli vysvětlena látka pomocí dříve popsaných bloků s vysvětlením látky \texttt{ExplanationStudyBlock}, jsou uživateli zobrazeny bloky \texttt{ExerciseStudyBlock}, které slouží k procvičení dané látky a sběru dat o tom, jak uživatel dané látce rozumí.

% Final
Každý tento blok je, podobně jako bloky s vysvětlením látky, vykreslen na samostatné stránce v rámci učení. Tento blok může obsahovat jeden nebo více komponentů \texttt{ExerciseStudyBlockComponent}, které jsou vykresleny pod sebou v jednom sloupci. Komponenty jsou buď pasivní, které pouze zobrazují informace, nebo aktivní, se kterými může uživatel interagovat. Pasivní komponenty jsou například nadpisy, odstavce nebo diagramy. Aktivními komponenty jsou různá cvičení, která musí uživatel vyplnit pro splnění daného bloku. Jeden \texttt{ExerciseStudyBlock} může obsahovat více aktivních komponentů. Příkladem toho je editor kódu, který obsahuje více oken s kódem. Každé okno je pak považováno za samostatný komponent cvičení.
...
% Final
\subsection{Vytvoření progresivní webové aplikace}

% Final
V rámci nefunkčního požadavku jsem doplnil konfiguraci aplikace nutnou pro to, aby prohlížeče aplikaci klasifikovaly jako progresivní webovou aplikaci. V souboru \texttt{manifest.ts} jsem definoval metadata aplikace, jako například název, jazyk aplikace nebo základní barvy. V rámci této konfigurace jsem také vytvořil několik základních ikon, které může progresivní webová aplikace využívat.

% Final
Dále jsem do aplikace doplnil registraci tzv. Service Worker skriptu. Service Worker je skript běžící v prohlížeči, jehož životní cyklus je takřka nezávislý na životním cyklu aplikace. Service Worker tak může běžet ve vlastním vlákně nezávisle na samotné aplikaci. Tato služba se využívá například pro správu cache aplikace, podporu offline funkcí a jako proxy mezi odchozími a příchozími požadavky a samotnou aplikací. V rámci této práce jsem se zmíněným funkcím nevěnoval, a proto jsem pouze zaregistroval prázdný Service Worker skript, který je nutný pro vytvoření PWA. \cite{ServiceWorker}
...
% Final
Díky výše zmíněné konfiguraci se tak aplikace stala progresivní webovou aplikací. Uživatelé si tak mohou aplikaci stáhnout a nainstalovat do svých zařízení přímo z prohlížeče na stránkách aplikace.

% Final
Aplikaci také bude možné v budoucnu po doplnění dalších konfigurací, například s pomocí nástroje PWABuilder \cite{PWABuilder}, zabalit a publikovat na platformách Google Play, App Store nebo například Microsoft Store. Publikace na tyto platformy je časově značně náročná a týká se z velké části spíše právních otázek nežli softwarového inženýrství. Proto se samotné publikaci v rámci této práce nebudu věnovat.

% Final
\subsection{Push notifikace}

% Final
Pro implementaci push notifikací jsem využil službu Firebase Cloud Messaging\footnote{Dostupné na: \href{https://firebase.google.com/products/cloud-messaging}{https://firebase.google.com/products/cloud-messaging}}. Tato služba umožňuje pro dané zařízení získat registrační token a následně z backendové části aplikace pomocí tohoto tokenu odesílat notifikace do daného zařízení.

% Final
Celý proces funguje následovně. Nejprve je uživateli zobrazeno upozornění \texttt{NotificationNotice}, které uživateli nabídne možnost zapnout notifikace na daném zařízení. Pokud uživatel možnost odmítne, je do lokálního úložiště zapsána informace o tom, že uživatel notifikace odmítl, a není mu pak toto upozornění zobrazováno znovu. Pokud uživatel možnost zapnutí notifikací přijme, prohlížeč mu ukáže potvrzovací dialog pro udělení oprávnění. Pokud uživatel oprávnění udělí, frontend aplikace pro dané zařízení získá pomocí služby Firebase Cloud Messaging registrační token a ten pak odešle backendové části aplikace. Backend pak toto zařízení registruje a může následně do zařízení začít odesílat push notifikace.

% Final
Pro příjem a zobrazování příchozích notifikací jsem rozšířil již existující Service Worker skript. Ten nově přijímá události typu \texttt{push}, které jsou vyvolané prohlížečem při příchodu push zprávy. Po přijetí zprávy následně tento skript pomocí rozhraní prohlížeče zobrazí notifikaci uživateli. Dále jsem doplnil skript o zpracování události \texttt{notificationclick}, která je vyvolána, když uživatel klikne na danou notifikaci. Po kliknutí na notifikaci skript přesměruje uživatele na hlavní stránku aplikace. Díky využití Service Worker skriptu je tak možné přijímat zprávy a zobrazovat notifikace i v situaci, kdy není aplikace otevřená, což je pro využití push notifikací zásadní.

% Final
Součástí této implementace bylo také zavedení nových nastavení pro notifikace. Uživatel si nyní může v nastavení aplikace vybrat, které typy notifikací chce přijímat. Tímto nastavením se pak řídí backendová část aplikace při odesílání jednotlivých push notifikací.
